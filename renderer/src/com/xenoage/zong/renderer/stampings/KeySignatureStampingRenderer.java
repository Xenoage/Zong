	...
	extends StampingRenderer
{
  ...
  @Override public void draw(Stamping stamping, Canvas canvas, RendererArgs args)
  {
    ...
    int fifth = s.traditionalKey.fifth;
    if (fifth == 0)
      return;
    boolean useSharps = (fifth > 0);
    float distance = (useSharps ?
    	s.layoutSettings.spacings.widthSharp : s.layoutSettings.spacings.widthFlat);
    Symbol symbol = args.symbolPool.getSymbol(
    ...
    {
      int linePosition = TraditionalKey.getLinePosition(i, useSharps, s.linePositionC4, s.linePositionMin);
      StaffSymbolStampingRenderer.drawWith(symbol, null,
      	sp(s.positionX + i * distance * interlineSpace, linePosition),
