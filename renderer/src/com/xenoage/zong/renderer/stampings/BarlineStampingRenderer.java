	...
	extends StampingRenderer
{
  ...
  @Override public void draw(Stamping stamping, Canvas canvas, RendererArgs args)
  {
    ...
    if (group == null || group == BarlineGroup.Style.Single || group == BarlineGroup.Style.Common)
    {
      ...
      {
      	xCorrection = paintBarline(canvas, args, staff, (staff.linesCount - 1) * 2,
    ...
    if (group == BarlineGroup.Style.Mensurstrich || group == BarlineGroup.Style.Common)
    {
      ...
      {
      	xCorrection = paintBarline(canvas, args, staves.get(i), 0,
          staves.get(i+1), (staves.get(i+1).linesCount - 1) * 2, xPosition, style);
      }
    }
   ...
   */
	private static void paintRepeatDots(List<StaffStamping> staves,
		float xPosition, int side, Canvas canvas, RendererArgs args)
	{
		for (StaffStamping staff : staves)
		{
    ...
    float xPosition, BarlineStyle style)
  {
  	...
  	float h = heavyMm;
  	float g = gapMm;
  	ColorInfo lightColor = col, heavyColor = col;
    ...
    //draw lines dependent on style
  	...
  	switch (style)
  	{
  		...
  		case HeavyLight: //heavy is centered (if barline not at the border of the staff)
  			if (isLeft) {c = h2; x += c;}
  			else if (isRight) {c = -l2 - h2 - g - l2; x += c;}
  			paintLine(canvas, args, staff1, staff1LP, staff2, staff2LP, x, h, heavyColor);
  			paintLine(canvas, args, staff1, staff1LP, staff2, staff2LP, x + h2 + g + l2, l, lightColor);
  			break;
  		case HeavyHeavy:
  			if (isLeft) {c = g2 + 2 * h2; x += c;}
  			else if (isRight) {c = -h2 - g2 - h2; x += c;}
   ...
   */
  private static void paintLine(Canvas canvas, RendererArgs args,
  ...
  private static void paintRepeatDot(Canvas canvas, RendererArgs args, StaffStamping staff, SP position)
  {
  	...
  	StaffSymbolStampingRenderer.drawWith(dotSymbol, ColorInfo.black, position, 1, staff, false,
  		canvas, args);
  }
