	...
	extends StampingRenderer
{
  ...
  @Override public void draw(Stamping stamping, Canvas canvas, RendererArgs args)
  {
    ...
    //draw line and hooks
    canvas.drawLine(
    	new Point2f(x1, y), new Point2f(x2, y), paintColor, paintWidth);
    if (volta.leftHook)
    {
    	canvas.drawLine(
      	new Point2f(x1, y), new Point2f(x1, y + hookHeight), paintColor, paintWidth);
    }
    if (volta.rightHook)
    {
    	canvas.drawLine(
      	new Point2f(x2, y), new Point2f(x2, y + hookHeight), paintColor, paintWidth);
    }
