	...
	extends StampingRenderer
{
  ...
  @Override public void draw(Stamping stamping, Canvas canvas, RendererArgs args)
  {
    ...
    float gapMm = 0;
    float textMm = 0;
    if (text != null && text.paragraphs.size() > 0)
    {
    	textMm = text.getFirstParagraph().getWidthMm();
    	gapMm = textMm * 2;
    }
    ...
    if (gapMm > 0)
    {
    	...
    	canvas.drawLine(
      	new Point2f(x1Mm, y1Mm), new Point2f(xGapLMm, yGapLMm), paintColor, paintWidth);
    	canvas.drawLine(
      	new Point2f(xGapRMm, yGapRMm), new Point2f(x2Mm, y2Mm), paintColor, paintWidth);
    }
    else
    {
    	//no gap
    	canvas.drawLine(
      	new Point2f(x1Mm, y1Mm), new Point2f(x2Mm, y2Mm), paintColor, paintWidth);
    }
    //hooks
    canvas.drawLine(
    	new Point2f(x1Mm, y1Mm), new Point2f(x1Mm, y1Mm + hookHeightPx * (tuplet.y1lp < 0 ? -1 : 1)), paintColor, paintWidth);
    canvas.drawLine(
    	new Point2f(x2Mm, y2Mm), new Point2f(x2Mm, y2Mm + hookHeightPx * (tuplet.y2lp < 0 ? -1 : 1)), paintColor, paintWidth);
    
    //draw text
    if (text != null && text.paragraphs.size() > 0)
    {
    	float textAscent = text.getFirstParagraph().ascent;
