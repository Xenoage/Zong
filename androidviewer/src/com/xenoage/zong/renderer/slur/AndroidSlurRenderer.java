...
public class AndroidSlurRenderer
{
    ...
    path.cubicTo(s.p2top.x + capDir.x, s.p2top.y + capDir.y,
    	s.p2bottom.x + capDir.x, s.p2bottom.y + capDir.y, s.p2bottom.x, s.p2bottom.y);
    //bezier curve back from p2bottom to p1bottom
    path.cubicTo(s.c2bottom.x, s.c2bottom.y, s.c1bottom.x, s.c1bottom.y, s.p1bottom.x, s.p1bottom.y);
    //cap at p1
    capDir = new Point2f(s.p1top.x - s.c1top.x, s.p1top.y - s.c1top.y).normalize().scale(cap);
    path.cubicTo(s.p1bottom.x + capDir.x, s.p1bottom.y + capDir.y,
