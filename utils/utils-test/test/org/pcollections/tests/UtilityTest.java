	...
	extends TestCase
{
	...
	public void testStack()
	{
		...
		assertEqualsAndHash("iteratively created list should be backwards", stack,
			reverse(stack2));

		PStack<String> stack3 = stack.plusAll(stack); // palindrome stack

		assertEqualsAndHash("plusAll should reverse order", stack3.subList(0, stack.size()),
			stack2);
	...
	public void testSet()
	{
		...
		int n = b.size();
		for (@SuppressWarnings("unused") String s : b)
			n--;
		...
		assertEqualsAndHash("adding a set to the empty set should be identity", b,
			MapPSet.from(HashTreePMap.<String, Object> empty(), b));

		// bug Mike found:
		assertEqualsAndHash(HashTreePSet.from(Arrays.asList(2)), HashTreePSet.empty().plus(1)
			.plus(2).minus(1));
	}
