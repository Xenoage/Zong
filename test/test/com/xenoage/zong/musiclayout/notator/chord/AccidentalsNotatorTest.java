...
public class AccidentalsNotatorTest
	...
	@Test public void testNoAcc() {
		//C5
		AccidentalsNotation accs = testee.compute(alist(Companion.pi(0, 0, 5)),
			new NoteDisplacement[] { note(5) }, cw, contextC);
		assertEmpty(accs);
		//C4, D4, G4
		accs = testee.compute(alist(Companion.pi(0, 0, 4), Companion.pi(1, 0, 4), Companion.pi(4, 0, 4)),
			new NoteDisplacement[] { note(-2), note(-1, noteOffset, susRight), note(2) }, cw, contextC);
		assertEmpty(accs);
		//Eb4, Ab4, G##5 with contextEb
		accs = testee.compute(alist(Companion.pi(2, -1, 4), Companion.pi(5, -1, 4), Companion.pi(4, 2, 5)),
