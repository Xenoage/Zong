...
import static com.xenoage.utils.base.iterators.MultiIt.multiIt;
...
public final class StavesListReader
{
  ...
  public static Value read(MxlScorePartwise mxlScore)
  {
    ...
    MxlPartList mxlPartList = mxlScore.scoreHeader.getPartList();
    ...
    for (MxlPartListContent mxlItem : mxlPartList.getContent())
    {
    	...
    	if (mxlItem.getPartListContentType() == PartListContentType.ScorePart)
      {
        ...
        Part part = PartReader.readPart(mxlScorePart,
        	findCorrespondingPart(mxlScorePart, mxlScore));
        parts = parts.plus(part);
        partsIDtoIndex = partsIDtoIndex.plus(mxlScorePart.getID(), currentPartIndex);
        currentPartIndex++;
      }
      //part-group
    ...
    if (!ArrayUtils.containsOnlyNull(openBarlineGroups) ||
    	!ArrayUtils.containsOnlyNull(openBracketGroups))
    {
  ...
  private static HashMap<String, Integer> countStaves(MxlScorePartwise mxlScore)
  {
    ...
    //check all parts
    for (MxlPart mxlPart : mxlScore.parts)
    {
      ...
      for (MxlMeasure mxlMeasure : mxlPart.getMeasures())
      {
        for (MxlMusicDataContent content : mxlMeasure.getMusicData().getContent())
        {
        	...
        	{
	          Integer xmlStaves = ((MxlAttributes) content).staves;
	          if (xmlStaves != null)
	          {
  ...
  {
  	for (MxlPart part : mxlScorePartwise.parts)
  	{
  		if (part.getID().equals(mxlScorePart.getID()))
  			return part;
  	}
  	throw new RuntimeException("There is no part for score-part \"" +
  		mxlScorePart.getID() + "\"");
  }
