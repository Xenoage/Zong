	...
	implements ScoreLayouterStrategy
{
		...
		LayoutSettings layoutSettings)
	{
		...
		for (int iMeasure : range(0, score.getMeasuresCount() - 1))
		{
			...
			for (ColumnElement element : score.header.getColumnHeader(iMeasure).getColumnElements())
			{
				for (int iStaff : range(measureColumn))
				{
					Notation notation = computeNotation(element,
						iStaff, score, symbolPool, layoutSettings);
					if (notation != null)
			...
			for (int iStaff : range(measureColumn))
			{
				...
				{
					Notation notation = computeNotation(element.element, iStaff,
						score, symbolPool, layoutSettings);
					if (notation != null)
						ret.add(notation);
				}
				for (int iVoice : range(measure.voices))
				{
					...
					{
						Notation notation = computeNotation(element, iStaff,
							score, symbolPool, layoutSettings);
						if (notation != null)
							ret.add(notation);
	 ...
	 */
	private Notation computeNotation(MusicElement element, int staff, Score score,
		SymbolPool symbolPool, LayoutSettings layoutSettings)
	{
		...
		if (element instanceof Chord)
			...
			notation = computeTraditionalKey((TraditionalKey) element,
				ScoreController.getClef(score, score.getBMP(element).withStaff(staff), Interval.Before),
				layoutSettings);
		...
		Score score, LayoutSettings layoutSettings)
	{
		...
		BMP bmp = score.getBMP(chord);
		MusicContext mc = getMusicContext(score, bmp, BeforeOrAt, Before);
		//compute the notation
		return computeChord(chord, mc, getInterlineSpace(score, bmp),
			score.format.lyricFont, stemDirection, globals, layoutSettings);
	}
	...
	private ChordNotation computeChord(Chord chord, MusicContext mc, float interlineSpace,
		FontInfo lyricsFont, StemDirection stemDirection, Globals globals, LayoutSettings layoutSettings)
	{
		//grace or normal chord?
		boolean grace = (chord.grace != null);
		...
		//notes alignment
		NotesAlignment notesAlignment = notesAlignmentStrategy.computeNotesAlignment(
		...
		AccidentalsAlignment accidentalsAlignment = accidentalsAlignmentStrategy.computeAccidentalsAlignment(
			chord, notesAlignment, chordWidths, mc);
		...
		StemAlignment stemAlignment = stemAlignmentStrategy.computeStemAlignment(chord.stem, notesAlignment,
			stemDirection, mc.getLinesCount(), grace ? layoutSettings.scalingGrace : 1);
	...
	*/
	...
	{
		return new ClefNotation(clef, new ElementWidth(0, ls.spacings.widthClef * ls.scalingClefInner, 0),
