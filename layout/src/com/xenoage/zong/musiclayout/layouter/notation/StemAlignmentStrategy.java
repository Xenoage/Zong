	...
	implements ScoreLayouterStrategy
{
   ...
   */
	public StemAlignment computeStemAlignment(Stem stem,
		NotesAlignment na, StemDirection sd, int linesCount, float scaling)
	{
		...
		if (stem != null && stem.length != null)
		{
			//used fixed length
			if (sd == Down)
			{
				endlineposition = low.getLinePosition() - 2 * stem.length;
			}
			else if (sd == Up)
			{
				endlineposition = high.getLinePosition() + 2 * stem.length;
			}
		}
		else
		{
