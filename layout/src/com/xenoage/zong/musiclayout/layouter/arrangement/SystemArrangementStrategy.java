	...
	implements ScoreLayouterStrategy
{
  	...
  	NotationsCache notations, ScoreLayouterContext lc)
  {
  	...
  	for (int iStaff : range(1, staffHeights.size() - 1))
  	{
  		...
  		if (staffLayout != null)
  		{
  			...
  			staffDistance = staffLayout.distance;
  		}
  		else
  		{
  			//use default staff distance
  			staffDistance = score.format.getStaffLayoutNotNull(iStaff).distance;
  		}
  		staffDistances.add(staffDistance);
  		totalStavesDistances += staffDistance; 
  	}
  	staffDistances.close();
    ...
    IVector<ColumnSpacing> systemMCSs = ivec();
    float usedWidth = 0;
    int currentMeasure;
    ...
    while (startMeasure + systemMCSs.size() < measuresCount)
    {
      ...
      { 
      	...
      	Tuple3<ColumnSpacing, VoiceSpacingsByStaff, NotationsCache> mcsData =
      		measureColumnSpacingStrategy.computeColumnSpacing(currentMeasure,
      			true /* leading! */, notations, lc);
      ...
      if (!canAppend(currentMCS, currentMeasure, usableWidth, usedWidth,
      	score.header, systemMCSs.size() == 0))
      {
        break;
      }
      else
  	...
  	float usableWidth, float usedWidth, ScoreHeader scoreHeader, boolean firstMeasureInSystem)
  {
  	...
  	Break br = scoreHeader.getColumnHeader(measureIndex).measureBreak;
  	if (br != null && br.systemBreak == SystemBreak.NewSystem && !firstMeasureInSystem)
  	{
  		return false;
  	}
  	
  	//if a line break is prohibited, force the measure to be within this system
  	boolean force = (br != null && br.systemBreak == SystemBreak.NoNewSystem);
	...
	private float getLeftMargin(int systemIndex, ScoreFormat scoreFormat, ScoreHeader scoreHeader)
	{
		...
		if (systemLayout != null)
		{
			...
			return systemLayout.marginLeft;
		}
		else
		{
			//use default system margin
			return scoreFormat.systemLayout.marginLeft;
		}
	}
	...
	private float getRightMargin(int systemIndex, ScoreFormat scoreFormat, ScoreHeader scoreHeader)
	{
		...
		{
			//use custom system margin
			return systemLayout.marginRight;
		}
		else
		{
			//use default system margin
			return scoreFormat.systemLayout.marginRight;
		}
	}
