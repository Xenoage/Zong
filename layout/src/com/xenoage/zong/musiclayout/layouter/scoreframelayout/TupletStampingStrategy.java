	...
	implements ScoreLayouterStrategy
{
		...
		SymbolPool symbolPool)
	{
		...
		for (Chord chord : tuplet.getChords())
		{
			...
			if (cs.stem != null)
			{
				stemDir += cs.stem.direction.getSignum();
			}
		}
	...
	private FormattedText createText(int digit, float fontSize, SymbolPool symbolPool)
	{
		...
		FormattedTextParagraph p = fPara(new FormattedTextSymbol(
			symbolPool.getSymbol(s), fontSize, FormattedTextStyle.defaultColor));
		return fText(p);
	}
