	...
	implements ScoreLayouterStrategy
{
	...
	public ChordStampings createChordStampings(ChordNotation chordNot,
		float positionX, StaffStamping staffStamping, Globals globals, SymbolPool symbolPool,
		LayoutSettings layoutSettings)
	{
		...
		IVector<NoteheadStamping> noteheads = ivec();
		IVector<LegerLineStamping> legerLines = ivec();
		IVector<ProlongationDotStamping> dots = ivec();
		IVector<AccidentalStamping> accidentals = ivec();
		IVector<ArticulationStamping> articulations = ivec();
    ...
    if (csd != StemDirection.None)
    {
    	...
    	if (stampNow)
    	{
	      ...
	      StemStamping stemSt = new StemStamping(staffStamping, chord,
	        stemX, stemStartPos, stemEndPos, csd);
	      stem = stemSt;
    	}
    	else
    	{
    ...
    for (int iNote : range(cna.getNotesCount()))
    {
      ...
      NoteheadStamping noteSt = new NoteheadStamping(chord, noteheadType, color, staffStamping,
      	sp(leftNoteX + na.getOffset() * interlineSpace, na.getLinePosition()), scaling, symbolPool);
    ...
    {
    	FlagsStamping.FlagsDirection flag = (csd == StemDirection.Up ?
        FlagsStamping.FlagsDirection.Down : FlagsStamping.FlagsDirection.Up);
      flags = new FlagsStamping(flag, flagsCount, scaling, staffStamping, chord,
      	sp(leftNoteX + cna.getStemOffset() * interlineSpace, stemEndPos), symbolPool);
    }
    ...
    if (caa != null)
    {
	    for (int iAcc = 0; iAcc < caa.getAccidentals().size(); iAcc++)
	    {
	      ...
	      AccidentalStamping accSt = new AccidentalStamping(chord, aa.getType(), staffStamping,
	        ...
	        aa.getLinePosition()), 1, symbolPool);
    ...
    for (int iNote : range(dotPositions))
    {
      for (int iDot : range(dotsPerNote))
      {
        ProlongationDotStamping dotSt = new ProlongationDotStamping(staffStamping, chord,
          sp(leftNoteX + cna.getDotsOffset(iDot) * interlineSpace, dotPositions[iNote]), symbolPool);
        dots.add(dotSt);
      }
    }
    ...
    if (cara != null)
    {
    	float noteheadWidth = chordWidths.get(chord.duration);
	    for (int iArt = 0; iArt < cara.getArticulations().length; iArt++)
	    {
	      ...
	      ArticulationStamping araSt = new ArticulationStamping(chordNot.getMusicElement(),
	      	...
	      	sp(leftNoteX + (ara.getXOffsetIS() + (noteheadWidth / 2)) * interlineSpace,
	      	ara.getYLP()), 1, symbolPool);
  	...
  	SymbolPool symbolPool)
  {
    return new ClefStamping(
      clef.getMusicElement(), staff, positionX, clef.getScaling(), symbolPool);
  }
  	...
  	StaffStamping staff, SymbolPool symbolPool, LayoutSettings layoutSettings)
  {
    return new KeySignatureStamping(
      key.getMusicElement(), key.getLinePositionC4(), key.getLinePositionMin(), positionX,
  	...
  	StaffStamping staff, SymbolPool symbolPool)
  {
  	...
  	{
  		return new CommonTimeStamping((CommonTime) time.getMusicElement(), positionX,
  			staff, symbolPool);
  	}
  	else
  	{
