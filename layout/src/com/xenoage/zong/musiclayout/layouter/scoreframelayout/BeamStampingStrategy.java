	...
	implements ScoreLayouterStrategy
{
	...
	enum Waypoint
	{
		None, Start, Stop, HookLeft, HookRight, StopHookRight
	}
		...
		Globals globals)
	{
		...
		if (beamedStems.getLastStem() == null)
		{
			throw new RuntimeException("Missing end stem for beam beginning at " +
				globals.getIMP((VoiceElement) beamedStems.getFirstStem().musicElement));
		}
		//compute beams
		...
		float leftLP = beamedStems.getFirstStem().endLp +
			beamedStems.getFirstStem().direction.getSignum() * lineHeight / 4; //4: looks ok
		float rightLP = beamedStems.getLastStem().endLp +
			beamedStems.getLastStem().direction.getSignum() * lineHeight / 4; //4: looks ok
    ...
    {
    	float leveledLP = -1 * beamedStems.getFirstStem().direction.getSignum() *
		...
		for (int iChord = 0; iChord <= beam.getWaypoints().size(); iChord++)
		{
			...
			if (startChord > -1 && stopChord > -1)
			{
				...
				if (chordsCount > 1)
				{
					...
					if (higherLevel == null || (higherLevel.get(stopChord) != Waypoint.HookRight
						&& higherLevel.get(stopChord) != Waypoint.StopHookRight))
