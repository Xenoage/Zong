	...
	implements ScoreLayouterStrategy
{
  	...
  	int iStaff, LayoutSettings layoutSettings)
  {
    ...
    IVector<SpacingElement> elements = ivec();
    ...
    ClefNotation clefNotation = new ClefNotation(
    	clef, new ElementWidth(0, layoutSettings.spacings.widthClef, 0), musicContext.getClef().getType().getLine(), 1);
    ...
    if (useKey)
    {
    	...
    	TraditionalKey tradKey = new TraditionalKey(((TraditionalKey) key).fifth); //it is not the same element instance, but has the same meaning
      TraditionalKeyNotation keyNotation =
      	notationStrategy.computeTraditionalKey(tradKey, clef, layoutSettings);
