...
public class BarlinesBeatOffsetsStrategy
{
	 ...
	 */
	public Tuple2<Vector<BeatOffset>, Vector<BeatOffset>> computeBeatOffsets(
		Vector<BeatOffset> baseOffsets, ColumnHeader columnHeader, float interlineSpace)
	{
		IVector<BeatOffset> retNotes = ivec(baseOffsets);
		IVector<BeatOffset> retBarlines = ivec();
		//start barline
		...
		for (BeatE<Barline> midBarline : columnHeader.middleBarlines)
		{
		...
		//return result
		return t((Vector<BeatOffset>) retNotes.close(), (Vector<BeatOffset>) retBarlines.close());
	}
