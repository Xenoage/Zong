	...
	implements ScoreLayouterStrategy
{
  	...
  	NotationsCache notations, Fraction measureBeats, LayoutSettings layoutSettings)
  {
    ...
    {
      return new VoiceSpacing(voice, interlineSpace, ivec(
        new SpacingElement(null, Fraction._0, 0),
        new SpacingElement(null, measureBeats, layoutSettings.spacings.widthMeasureEmpty)).close());
    }
    ...
    for (VoiceElement element : reverseIt(voice.elements))
    {
      ...
      if (!grace)
      {
        ...
        symbolOffset = Math.min(lastFrontGapOffset - layoutSettings.spacings.widthDistanceMin,
        	lastFullSymbolOffset - elementWidth.getRearGap()) -
      ...
      else
      {
      	...
      	symbolOffset = Math.min(lastFrontGapOffset,
      		lastSymbolOffset - elementWidth.getRearGap()) - elementWidth.getSymbolWidth();
      }
    ...
    IVector<SpacingElement> ret = ivec();
    for (SpacingElement se : acc)
      ret.add(se.withOffset(se.offset + shift));
    	
    return new VoiceSpacing(voice, interlineSpace, ret.close());
  }
