...
import static com.xenoage.utils.base.collections.SortedList.sortedListNoDuplicates;
	...
	implements ScoreLayouterStrategy
{
  ...
  public PVector<BeatOffset> computeBeatOffsets(
  	VoiceSpacingsByStaff voiceSpacings, Fraction measureBeats,
  	LayoutSettings layoutSettings)
  {
  	...
  	for (int iStaff : range(voiceSpacings.staves))
  	{
  		for (int iVoice : range(voiceSpacings.staves.get(iStaff)))
  		{
  			...
  			if (vs == null)
  				throw new IllegalStateException("VoiceSpacing for voice at (staff " +
  					iStaff + ", voice " + iVoice + ") is missing!");
  			vss.add(vs);
  		}
  	}
  	//compute voice spacings
  	return computeBeatOffsetsFromVoiceSpacings(vss, measureBeats, layoutSettings);
  }
  	...
  	LayoutSettings layoutSettings)
  {
    ...
    LinkedList<BeatOffset> ret = llist();
    ...
    if (voiceSpacings.size() == 1)
    {
      ...
      for (SpacingElement se : voiceSpacings.getFirst().getSpacingElements())
      {
      	...
      	if (ret.getLast().getBeat().equals(se.beat))
      		ret.removeLast();
   ...
   */
  Fraction computeLastBeat(LinkedList<VoiceSpacing> voiceSpacings)
  {
    Fraction ret = _0;
    for (VoiceSpacing voiceSpacing : voiceSpacings)
    {
