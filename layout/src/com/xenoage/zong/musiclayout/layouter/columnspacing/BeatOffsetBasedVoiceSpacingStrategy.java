	...
	implements ScoreLayouterStrategy
{
  ...
  public VoiceSpacing computeVoiceSpacing(VoiceSpacing voiceSpacing,
  	Vector<BeatOffset> beatOffsets)
  {
  	Vector<SpacingElement> oldSpacingElements = voiceSpacing.getSpacingElements();
    IVector<SpacingElement> spacingElements = ivec(oldSpacingElements);
    ...
    for (int iGivenBeat = 0; iGivenBeat < sharedBeats.size(); iGivenBeat++)
    {
      ...
      for (int iElement = lastElement; iElement >= firstElement; iElement--)
      {
      	...
      	else
      	{
      		...
      		spacingElements.set(iElement, e.withOffset(
      			spacingElements.get(iElement+1).offset - distanceBefore));
      	}
      }
    ...
    return new VoiceSpacing(voiceSpacing.getVoice(),
    	voiceSpacing.getInterlineSpace(), spacingElements.close());
  }
