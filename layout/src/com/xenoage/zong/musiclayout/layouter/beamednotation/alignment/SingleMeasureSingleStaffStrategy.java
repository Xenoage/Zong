 ...
 * which can only compute beams that are all in the same measure and staff.
 * 
	...
	implements ScoreLayouterStrategy
{
		...
		ColumnSpacing columnSpacing, NotationsCache notations)
	{
		...
		MP firstChordMP = globals.getIMP(firstChord);
		...
		StemDirection dir = notations.getChord(firstChord).getStemDirection();

		//compute the stem alignments
		BeamStemAlignments bsa = computeStemAlignments(chordNa, stemX, staffLinesCount,
			beamLinesCount, dir);
	...
	public BeamStemAlignments computeStemAlignments(NotesAlignment[] chordNa,
		float[] stemX, int staffLinesCount, int beamLinesCount, StemDirection stemDirection)
	{
		...
		switch (beamLinesCount)
		{
			...
			case 3:
				...
				break;
			default:
				beamDesign = new MultipleBeamDesign(stemDirection, staffLinesCount,
					beamLinesCount);
		}
		...
		float[] positionX, StemDirection stemDirection, int staffLinesCount)
	{
		...
		if (useDefaultSlant)
		{
			...
			return computeNormalBeamSlant(beamDesign, firstRelevantNoteLP, lastRelevantNoteLP,
				lengthX, staffLinesCount);
		}
	 ...
	 * @param staffLinesCount  the number of lines in this staff
	 */
	private float computeNormalBeamSlant(BeamDesign beamDesign, int firstNoteLP,
		int lastNoteLP, float lengthX, int staffLinesCount)
	{
		...
		StemDirection stemDirection)
	{
		...
		float beamStartLPCorrection = stemDirection.getSignum() * 0.5f;
	 ...
	 */
	public static boolean isBeamOutsideStaff(StemDirection stemDirection,
		float firstStemEndLP, float lastStemEndLP, int staffLinesCount, float totalBeamWidthIS)
	{
		...
		else if (stemDirection == StemDirection.Down)
		{
			...
			//beam lines below the staff?
			if (firstStemEndLP < -1 * totalBeamWidthIS * 2
				&& lastStemEndLP < -1 * totalBeamWidthIS * 2)
			{
				return true;
			}
		}
