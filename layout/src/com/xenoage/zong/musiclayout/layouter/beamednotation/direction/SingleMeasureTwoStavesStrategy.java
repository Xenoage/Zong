	...
	implements ScoreLayouterStrategy
{
	...
	public NotationsCache computeNotations(Beam beam,
		NotationsCache notations, Score score, LayoutSettings layoutSettings)
	{
		//do the work
		BeamStemDirections bsd = computeBeamStemDirections(beam, score);
		...
		for (int i = 0; i < bsd.getStemDirections().length; i++)
		{
			...
			{
				ret.add(notationStrategy.computeChord(chord,
					bsd.getStemDirections()[i], score, layoutSettings));
			}
		}
		return ret;
	}
   ...
   */
  BeamStemDirections computeBeamStemDirections(Beam beam, Score score)
  {
  	StemDirection[] ret = new StemDirection[beam.getWaypoints().size()];
  	int upperStaffIndex = beam.getUpperStaffIndex(score.globals);
  	It<BeamWaypoint> waypoints = it(beam.getWaypoints());
  	for (BeamWaypoint waypoint : waypoints)
		{
			int staffIndex = score.getBMP(waypoint.getChord()).staff;
