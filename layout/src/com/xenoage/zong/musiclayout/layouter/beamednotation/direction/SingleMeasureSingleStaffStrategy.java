	...
	implements ScoreLayouterStrategy
{
	...
	public NotationsCache computeNotations(Beam beam,	NotationsCache notations,
		int linesCount, Score score, LayoutSettings layoutSettings)
	{
  	...
  	StemDirection[] stemDirections, int linesCount)
  {
    ...
    for (int iChord = 0; iChord < chordsLp.length; iChord++)
    {
      ...
      {
        up++;
        if (Math.abs(middlelinepos-chordLp.getTop()) > Math.abs(middlelinepos - chordsLp[furthest].get(chordsLp[furthest].getNotesCount()-1)))
        {
          furthest = f;
        }
      }
      ...
      {
        down++;
        if (Math.abs(middlelinepos-chordLp.getBottom()) > Math.abs(middlelinepos - chordsLp[furthest].get(chordsLp[furthest].getNotesCount()-1)))
        {
          furthest = f;
        }
      }
