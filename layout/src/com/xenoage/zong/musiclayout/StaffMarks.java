 ...
 * 
 * It contains also the relevant {@link BMP}s
 * and their horizontal positions within the staff in mm.
 * Coordinates can be converted to {@link BMP}s and backwards.
...
public final class StaffMarks
{
	...
	private final Vector<MeasureMarks> measureMarks;
	 ...
	 */
	public StaffMarks(int systemIndex, int staffIndex, int startMeasureIndex, Vector<MeasureMarks> measureMarks)
	{
	...
	public BMP getMPAt(float xMm)
	{
		...
		return bmp(staffIndex, measureIndex, 0, beat);
	}
	 ...
	 */
	public Vector<MeasureMarks> getMeasureMarks()
	{
		return measureMarks;
	}
