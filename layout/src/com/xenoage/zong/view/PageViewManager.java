...
public class PageViewManager
{
	...
	private ArrayList<Rectangle2f> computePageRects()
	{
		...
		List<Page> pages = layout.pages;
		ArrayList<Rectangle2f> ret = new ArrayList<Rectangle2f>(pages.size());
		if (pages.size() > 0)
		{
			// compute offsets for all pages
			float firstPageOffsetX = -pages.get(0).format.size.width / 2;
			float firstPageOffsetY = -pages.get(0).format.size.height / 2;
			for (int i = 0; i < pages.size(); i++)
			{
				Size2f pageSize = pages.get(i).format.size;
				Point2f offset = null;
	...
	{
		Point2f pMm = new Point2f(
			Units.pxToMm(pPx.x - viewState.sizePx.width / 2, viewState.scaling),
	...
	{
		int pageIndex = layout.pages.indexOf(page);
	...
	public Point2i computePositionPx(LP p, ViewState viewState)
	{
		...
		Point2i ret = new Point2i(
			Units.mmToPxInt(pos.x, viewState.scaling),
			Units.mmToPxInt(pos.y, viewState.scaling));
  ...
  public Rectangle2i computePageRect(int pageIndex, ViewState viewState)
  {
    ...
    Point2i pagePositionUpperLeftPx = computePositionPx(
      lp(layout, pageIndex, p(0, 0)), viewState);
    Size2f size = layout.pages.get(pageIndex).format.size;
