 ...
 * Currently, AWT apps use the {@link GeneralPath} class, while
 * Android apps use the <code>android.graphics.Path</code> class.
 * 
 * There are no subclasses to make things not more complicated than needed.
 *
  ...
  extends Symbol
{
  ...
  public PathSymbol(String id, Object path, Rectangle2f boundingRect,
    Float baselineOffset, Float ascentHeight, Float leftBorder, Float rightBorder)
  {
  	super(id, boundingRect, baselineOffset,
  		ascentHeight, leftBorder, rightBorder);
  	this.path = path;
  }
