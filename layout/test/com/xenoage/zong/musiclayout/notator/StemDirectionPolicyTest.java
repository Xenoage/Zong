...
/**
 * Tests for {@link StemDirectionPolicy}.
 * 
 * @author Andreas Wenger
 */
public class StemDirectionPolicyTest {

	StemDirectionPolicy testee = stemDirectionPolicy;
	...
	@Test public void computeStemDirectionTest() {
		...
		for (int i = -2; i <= 3; i++) {
			lps[0] = i;
			assertEquals(Up, testee.computeStemDirection(clp(lps), linesCount));
		}
		for (int i = 4; i <= 13; i++) {
			lps[0] = i;
			assertEquals(Down, testee.computeStemDirection(clp(lps), linesCount));
		}
