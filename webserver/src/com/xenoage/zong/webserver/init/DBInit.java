...
public class DBInit
{
		...
		throws SQLException
	{
		...
		sql(con,
			"create domain audioformat as varchar default 'OGG' check value in('OGG',	'MP3')");
		//table "docs": list of documents which are currently stored
		sql(con,
			"create table docs (" +
			"id int not null auto_increment, " + //ID of the document
		...
		sql(con,
			"create table audio (" +
			"doc_id int not null, " + //ID of the document
		...
		sql(con,
			...
			"doc_id int not null, " + //ID of the document (table docs)
